...
- **Start worker** launches an agent (from **Settings → AI Providers**) on a ready task, in its
  own session and, optionally, its own git worktree. That launch is a **dispatch**.
...
3. **Neither.** A liveness sweep every 15 seconds marks a worker **lost** as soon as its window
...
Workers are tracked by **window id**, so renaming a session or window changes nothing.
...
```sh
export TS_AGENT_SOCK=... TS_RUN_ID=... TS_TASK_ID=... TS_DISPATCH_ID=...; export PATH='<config>/perch/bin':"$PATH"; <agent launch command> '<one-line brief>'
```
...
Installed to `<config>/perch/bin/agent-task` whenever the extension activates, and put on
...
`TS_AGENT_SOCK=~/.config/perch/agent-tasks/control.sock`.
...
All under `~/.config/perch/` (or `$XDG_CONFIG_HOME/perch/`):
...
user who runs perch. Anything that user runs can create runs and tasks and start workers
