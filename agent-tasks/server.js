...
// one agent, launched into its own session (and optionally its own
// worktree), tracked by the window id recorded at launch.
//
...
const HOME = homedir();
const configDir = path.join(process.env.XDG_CONFIG_HOME || path.join(HOME, ".config"), "perch");
...
export function activate({ router, log = console.log, getSettings, host }) {
  ...
  // Every window the backend knows, by id. `certain` is false when some session could
  ...
  async function startWorker(body) {
    if (!host.sessions || !host.worktrees || !host.agents?.launchCommand) {
      throw new HttpError(501, "this perch is too old to launch workers - update it (needs host.sessions)");
    }
    const taskId = str(body.taskId, "taskId", { required: true });
  ...
  async function cleanupWorker(body) {
    if (!host.worktrees?.remove) throw new HttpError(501, "this perch is too old to remove worktrees - update it");
  ...
  async function state() {
    ...
    // what a finished worker left behind. null when the session list could not be read.
  ...
  (async () => {
    ...
    if (host.sessions) {
      ...
      log("this perch has no host.sessions: workers cannot be launched or swept - update it");
    }
  })().catch((err) => log("activation failed:", err?.stack ?? err));
}

// The CLI a worker runs, copied to <config>/perch/bin/agent-task (the
