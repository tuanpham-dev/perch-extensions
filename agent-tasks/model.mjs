...
// terminal backend, no filesystem, no clock of its own (every "now" is a parameter), so
...
//
// Dispatches are keyed on the window id recorded at launch, never on a
...
// knows only its window id) finds the work it belongs to.
