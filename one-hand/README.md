...
A transparent **gesture bar** across the bottom of the editor — on *every* tab (terminal, Settings, viewers) — for driving Perch one-handed from a phone or tablet. First-party (Perch), MIT-licensed.
...
Needs a Perch build that provides the `registerAppOverlay` extension point and the `app.executeCommand` / `app.getCommands` context APIs.
