...
// implemented against a Perch, so sessions made with `tmux new` in any
...
// `socketName` and `configFile` pick a separate Perch (tmux -L, -f), for
...
export function createTmuxEngine({ env = process.env, socketName = "", configFile = "", bin = "tmux", log = () => {} } = {}) {
  ...
  // Variables every new pane gets on top of the Perch's own environment.
  // PERCH_WINDOW is cleared: a Perch started inside one of the
  ...
  const paneEnvArgs = ["-e", "PERCH_WINDOW="];
  for (const name of ["BROWSER", "PERCH_PORT"]) {
  ...
  // `styles` adds -e: colors and attributes stay in as SGR escapes.
  async function capture(target, scrollback = 0, opts = {}) {
    const { paneId } = await resolve(target);
    const lines = Math.max(0, Math.floor(Number(scrollback) || 0));
    const args = ["capture-pane", "-p", "-J", ...(opts?.styles ? ["-e"] : []), "-t", paneId];
