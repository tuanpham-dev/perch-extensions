// The tmux backend against a real Perch of its own (a private socket,
...
before(() => {
  ...
  engine = createTmuxEngine({
    socketName,
    configFile: "/dev/null",
    env: { PATH: process.env.PATH, HOME: dir, SHELL: "/bin/sh", ENV: "", PS1: "$ ", TERM: "xterm-256color", BROWSER: "/x/open", PERCH_PORT: "4321" },
  });
});
...
test("sessions and windows made in the app are tmux's, and changes show up both ways", { skip: !hasTmux }, async () => {
  assert.deepEqual(await engine.listSessions(), [], "no Perch yet is an empty listing");
