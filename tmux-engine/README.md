...
Runs Perch's terminals in [tmux](https://github.com/tmux/tmux) instead of the bundled terminal daemon. Sessions you start with `tmux new` in any terminal show up in the app, and sessions started in the app are ordinary tmux sessions you can `tmux attach` to.
...
- Extensions can read a pane's screen as text (`host.sessions.capture`), which comes from `capture-pane`. Needs a Perch version with that host call; older versions simply never ask.
...
Requires tmux 3.2 or newer (tested with 3.5a). `PERCH_TMUX_SOCKET` in the server's environment names a separate tmux socket (`tmux -L`), which is how tests and test instances avoid touching your own tmux.
