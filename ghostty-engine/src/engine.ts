...
import { cellFromPoint, joinedSelectionText, markSyntheticSelectStart } from "@perch/engine-support";
...
export async function createGhosttyEngine(options: TerminalEngineOptions): Promise<TerminalEngineHandle> {
  ...
  const handle: TerminalEngineHandle = {
    ...
    // scrollback; the server owns it), ghostty-web's scrollback counter keeps
    ...
    // Always false: this app never scrolls ghostty locally — the server owns all
    // scrollback, and a touch-swipe/wheel scrolls the server-side scrollback, not this
