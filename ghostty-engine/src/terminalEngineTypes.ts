...
export interface TerminalEngineSettings {
  ...
  // the host's client/src/selectionText.ts (via @perch/engine-support).
...
export interface TerminalEngineHandle {
  ...
  // engine's own buffer — the server owns real scrollback, so this only ever
  // reflects a momentary local scroll, not the server-side scrollback.
