...
export interface TerminalLinksHandlers {
  ...
  // can activate it directly without letting a ctrl+click reach the terminal.
...
export function buildLinkProvider(term: Terminal, handlers: TerminalLinksHandlers): ILinkProvider {
  return {
    ...
    // history get no links — the local viewport never scrolls
    // anyway (the server owns scrollback).
