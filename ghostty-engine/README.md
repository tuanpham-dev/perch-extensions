...
rendering engine, packaged as an installable Perch extension.

Installing it adds a **Ghostty** option under **Settings → Terminal**. When it
isn't installed, Perch uses its bundled, required **xterm.js** engine —
so this is purely an opt-in upgrade, and uninstalling it always falls back
cleanly.

It contributes a terminal engine (`ext.perch.ghostty-engine.ghostty`) and
