#!/usr/bin/env node
// Validates every extension manifest in this repo against what Perch's
...
// registry for every Perch install (server/src/registry.ts hardcodes its
...
// R3/R4/R5 — the files pack.mjs copies into the catalog beside the .perch.
...
function checkEntries(name, manifest) {
  const entry = manifest.perch;
  if (!entry) return;
  if (typeof entry.client === "string") {
    if (existsSync(path.join(repoRoot, name, "src/client.tsx"))) {
      if (entry.client !== "./dist/client.js") {
        fail(name, "R7", `has src/client.tsx, so perch.client must be "./dist/client.js" (found ${JSON.stringify(entry.client)})`);
      }
    } else if (!existsSync(path.join(repoRoot, name, entry.client))) {
      fail(name, "R7", `perch.client "${entry.client}" does not exist and there is no src/client.tsx to build it from`);
    }
  }
  if (typeof entry.server === "string" && !existsSync(path.join(repoRoot, name, entry.server))) {
    fail(name, "R7", `perch.server "${entry.server}" does not exist`);
  }
  if (entry.required === true) {
    fail(name, "R7", 'perch.required is bundled-only; the host ignores it on installed extensions');
  }
}
