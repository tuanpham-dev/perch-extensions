...
Run Linux GUI apps on the server and view/control them right inside Perch, streamed over [xpra](https://xpra.org/) — an adaptive, low-latency HTML5 remote display. Much faster than X11 forwarding (`ssh -X`) or plain VNC: xpra sends only the screen regions that changed, encoded per-region (jpeg/webp for UI, video codecs for motion) instead of polling full frames.
...
2. **Start GUI Session** — brings up an xpra display in the background, in a dedicated `gui-apps` session that shows up like any other session; open it to read xpra's own output, which is also where to look if the display fails to start. Works on the bundled terminal daemon and on the tmux Terminal Backend alike.
