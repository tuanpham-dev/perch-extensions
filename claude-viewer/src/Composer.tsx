...
export const Composer = forwardRef<ComposerHandle, {
  ...
  working?: boolean;
  onStop?: () => void;
}>(function Composer({ commands, searchFiles, onSend, disabled, disabledReason, terminalInput, enterSends, working, onStop }, ref) {
    ...
    <div className="cv-composer">
      ...
      <div className="composer">
        <button
          ...
          title="Attach files (or drop them anywhere on this tab)"
        ...
        {empty && working && onStop ? (
          <button className="btn btn-stop cv-send" onClick={onStop} disabled={disabled} title="Interrupt Claude (Esc)">
            Stop
          </button>
        ) : empty && offered ? (
          <button
            className="btn btn-primary cv-send"
            onClick={() => {
              setInput(offered);
              textareaRef.current?.focus();
            }}
            disabled={disabled}
            title="Put the text from the terminal's input here (Tab)"
          >
            Use
          </button>
        ) : (
          <button className="btn btn-primary cv-send" onClick={() => void send()} disabled={empty || sending || disabled || uploading > 0} title={sendKeyHint}>
            Send
          </button>
        )}
      </div>
