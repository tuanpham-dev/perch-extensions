...
function loadChunk(): Promise<Chunk> {
  if (!host.assetUrl) return Promise.reject(new Error("not active"));
  chunk ??= import(/* @vite-ignore */ host.assetUrl("dist/chunks/highlight.js")) as Promise<Chunk>;
  return chunk;
}
...
function readTheme(): HostTheme {
  const colors = host.app?.getThemeColors?.() ?? {};
  const tokenColors = (host.app?.getTokenColors?.() ?? []) as unknown[];
...
function subscribeTheme(cb: () => void): () => void {
  ...
  if (!unsubscribeHost && host.app?.onDidChangeColorTheme) {
    unsubscribeHost = host.app.onDidChangeColorTheme(() => {
      currentTheme = null;
      themeVersion++;
      cache.clear();
...
export function CodeBlock({ code, lang }: { code: string; lang: string | null }) {
  ...
  return (
    <pre className="cv-code">
      <code>
        {lines.map((line, i) => (
          <div key={i} className="cv-code-line">
            {tokens?.[i] ? <TokenLine tokens={tokens[i]} /> : line || " "}
          </div>
        ))}
      </code>
    </pre>
