...
- **Tool calls** as cards with a preview of what the terminal would show: a highlighted diff for Edit and Write, output lines for Bash, a count for Read, matches for Grep and Glob, the latest steps of a running subagent (its report once it is done), and a thumbnail for screenshots and images. Expand a card for its full input and result; an Agent card expands to the prompt it was given and its report.
- **Subagents** at full size. **Open transcript** on an Agent card shows that subagent's own conversation in place of the main one, with the same cards, images and file links. Its header has a Back button, the path from the main conversation (Main › general-purpose › Explore, each part clickable), and the agent's type, description, status, model, elapsed time, steps and tokens. A subagent started by a subagent opens the same way from inside it. The message box, prompts and footer stay as they are while you read one, and sending a message takes you back to the main conversation.
- **Every subagent in the conversation** from the chip in the footer, which reads "3 agents · 1 running". It opens a list with the same details, nested agents indented under the one that started them; pick one to open it. Esc or a click elsewhere closes the list.
- **Running or finished, correctly.** Claude Code runs subagents in the background, so its Agent call returns at once. An agent counts as running until Claude Code reports it finished, failed or stopped, and one you send a new message to runs again. Where that report arrives, the conversation shows one line, such as `● Agent "Find hook callers" finished`, which opens the agent. Once finished, its tokens, steps and time are Claude Code's own totals.
...
- **Send messages**: several lines at once, `/` for commands and skills, `@` for files in the project, and files pasted, attached or dropped anywhere on the tab (images show as thumbnails; other files are uploaded and their path added to the message). Anything already typed in the terminal's input box shows as the composer's placeholder while it is empty; press Tab or the **Use** button to take it into your message. While Claude is working and the composer is empty, its button is **Stop**, which interrupts the turn like Esc. A message sent while Claude is working is queued by Claude Code and picked up inside the running turn, so it appears in the conversation between that turn's tool calls rather than starting one, marked "Sent while Claude was working".
