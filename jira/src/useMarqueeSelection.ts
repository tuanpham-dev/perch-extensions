// Rubber-band marquee selection, used by the issue list's drag-select.
// Copied verbatim from Perch core's client/src/hooks/useMarqueeSelection.ts
// (via its extensions/_shared copy) - extension code can't reach client/src,
// and this registry repo can't import across extensions either, so it is
// vendored with this comment naming the source rather than silently
// duplicated. Same reason agentTarget.ts is a copy; see its header.
//
// Mouse only, by design: touch uses useLongPressMenu beside this file to
// begin a selection, because a drag on a touch screen has to stay available
