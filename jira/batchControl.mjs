// The control socket: how a cluster's agent, from inside its own terminal
// window, reports back to this extension.
//
...
// body, answered with JSON (or text, for `brief`). curl speaks it with
// --unix-socket, which is all cli/jira-batch is.
//
...
// whole server on one.
//
// Lifted from the agent-tasks extension's control.mjs, which solved the same
// problem first. A copy rather than an import: extensions are packaged and
// installed separately, so there is no module either could share.
...
export function createControlServer({ socketPath, handlers, log = console.log }) {
  ...
  async function handle(req, res) {
    try {
      ...
      if (!handler) throw new ControlError(404, `unknown verb "${verb}" - run jira-batch help`);
